...
namespace AdventureGame
{
    ...
    public class Player : Actor
    {
        ...
        public Player(string name, int health = 20, Locale location = Locale.Nowhere) : base(name, health, location)
        {
            ...
            this.Bag.Add(GameItems.RabbitsFoot, GameItems.RabbitsFoot.Money);
        ...
        public void BuyItem(Item item)
        {
            ...
            int amountSpentByPlayer = SpendMoney(item.Money);
            AddItem(item);   

            if (amountSpentByPlayer > item.Money)
            {
                AddItem(new Item("Cash Back", amountSpentByPlayer - item.Money));
            }
        ...
        public string ToString()
        {
            ...
            foreach (Item item in this.Bag.Keys)
            {
                ...
                {
                    str += $"{comma}[{item.Name}(${item.Money})]";
                    comma = ", ";
                }
            }
